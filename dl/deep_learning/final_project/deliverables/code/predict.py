...
model.load_weights('./no_rot.hdf5')
...
im6 = cv2.resize(cv2.imread('frame0105.jpg'), (224, 224)).astype(np.float32)
...
im9=cv2.resize(cv2.imread('frame0122.jpg'),(224,224)).astype(np.float32)
im_list = [im,im1,im2,im3,im4,im5,im6,im7,im8,im9]
# im[:,:,0] -= 103.939
# im[:,:,1] -= 116.779
# im[:,:,2] -= 123.68
cnt = 1
